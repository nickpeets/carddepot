...
(function () {
  ...
  function mountChip() {
    return getBalance().then(function (bal) {
      var host = chipHost();
      if (!host) { console.warn(TAG, 'no [data-depot-franchise] host; chip not mounted'); return; }
      // Idempotent: strip any previously-mounted chip so auth re-mounts render exactly one
      // (never clear the whole plate host.innerHTML - that wipes team/record/coins).
      var slot = host.querySelector('[data-depot-wallet]') || host;
      var existing = slot.querySelectorAll('.depot-wallet-chip');
      for (var k = 0; k < existing.length; k++) { existing[k].parentNode.removeChild(existing[k]); }
      if (bal == null) { console.warn(TAG, 'wallet chip hidden until balance resolves'); return; }
      slot.insertAdjacentHTML('beforeend',
        '<span class="depot-wallet-chip v2-coin-pill" title="Franchise balance">' +
          '<span class="v2-coin dw-coin" aria-hidden="true">' +
            '<span class="dw-coin-inner">' +
              '<span class="dw-coin-face dw-coin-front"><span class="dw-coin-sym">$</span></span>' +
              '<span class="dw-coin-face dw-coin-back"><span class="dw-coin-sym">$</span></span>' +
            '</span>' +
            '<span class="dw-coin-glint" aria-hidden="true"></span>' +
          '</span>' +
