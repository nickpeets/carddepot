# ECONOMY_DESIGN.md â The Franchise Economy
...
> Part 2 DDL is **proposed for Nick to run** â no schema/RLS executed by this session
> (AGENTS.md 2). Currency name placeholder: **DEPOT DOLLARS (DD)** â Nick's to rename.
>
...
**STAR tier** â from a static, repo-shipped table data/player_tiers.json
...
**ROOKIE bonus: +30** â awarded when card year == player's MLB debut year.
...
**VINTAGE bonus** â by era bucket of the card year:

| Era bucket        | Points | Label   |
|-------------------|-------:|---------|
| pre-1980          |     20 | VINTAGE |
| 1980â1989         |     10 | VINTAGE |
| 1990â1994         |      6 | ERA     |
| 1995 and later    |      0 | â       |

**GEM bonus: +15** â a **manual flag** (gem bool, admin/owner-set). Off by default;
a hook for standout copies (autos, rare parallels) without any market data.

**Floor:** any real card scores at least **5** (a commons-bin modern card sits at 5â8).

### 1.1 Worked example â the mockup's legibility target
...
GOLD >= 60 Â· SILVER 30â59 Â· BRONZE 10â29 Â· (below 10 = plain badge).
...
HOF vintage stars land ~46â50; Griffey is the ceiling at 80; commons at 5â8. Full
per-card table for all 18 cards is produced live in the session report.

### 1.4 The tier table â data/player_tiers.json, how it grows
Shipped seed keyed by normalized name (lowercase, NFKD, punctuation-stripped â same
...
### 1.5 Amendment (hobby-research corrections) — supersedes the era model in §1 and the §1.1 example

Real-hobby feedback tightened the formula. Where this section conflicts with the
VINTAGE table above or the §1.1 worked example, **this section wins**; the engine
...
Money is paid on game completion:
    ...
    WIN  payout = BASE_WIN (100) + round(lineup_prestige Ã 1.8) + bonuses
...
- UNBEATEN STREAK: **+15 Ã current win streak** (capped at Ã8 = +120)

**Scale check (the builder/payday mockup):** lineup_prestige **440** ->
100 + round(440 Ã 1.8 = 792) = **892** base win purse -> the builder's
"LINEUP PRESTIGE 440 -> WIN PAYS ~889" (rounded). With the mockup's stacked bonuses
(shutout +50, streak Ã4 = +60) the payday = **$1,002 DD**, new balance $2,129 â
